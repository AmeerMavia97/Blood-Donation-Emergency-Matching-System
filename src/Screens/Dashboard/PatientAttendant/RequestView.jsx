...
import { CheckCircle2, Copy, Droplet, Hospital, Link as LinkIcon, PhoneOff } from "lucide-react"
    ...
    <DashboardLayout roleName={roleName} tabs={patientTabs} title="Request Details" description="View request verification, matched donors, blood bank offers, and share link.">
      ...
      {data && (
        <div className="space-y-6">
          ...
          <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            <div className="rounded-[2rem] border border-black/10 bg-white p-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Info label="Blood Group" value={data.required_blood_group} />
                <Info label="Units" value={`${data.units_required} Units`} />
                <Info label="Hospital" value={data.hospital_name || "Pending"} />
                <Info label="Verification Status" value={data.verification_status || "Pending"} />
              </div>
            ...
            <div className="rounded-[2rem] bg-[#151515] p-6 text-white">
              ...
              <div className="mt-6 space-y-4">
                {timelineSteps.map((step, index) => {
                  const completed =
                    index < 2 || // first two steps always complete
          ...
          {/* Matched Donors + Blood Bank + Share Link */}
...
const Info = ({ label, value }) => (
  <div className="rounded-2xl bg-[#f7f7f7] p-4">
    <p className="mb-1 flex items-center gap-2 text-xs font-black uppercase tracking-wide text-[#dc2626]">{label}</p>
...
const Donor = ({ full_name, blood_group, consent_phone, phone }) => (
  <div className="mb-3 flex items-center gap-3 rounded-2xl bg-[#f7f7f7] p-4">
    <span className="flex size-10 items-center justify-center rounded-full bg-[#dc2626] text-xs font-black text-white">{blood_group}</span>
    <div>
      <p className="font-head text-lg font-black text-[#151515]">{full_name}</p>
      {consent_phone && <p className="text-xs font-semibold text-[#555]">📞 {phone}</p>}
      {!consent_phone && <p className="text-xs font-semibold text-[#555]"><PhoneOff size={12} className="inline mr-1" />Phone hidden until donor consent</p>}
    </div>
  </div>
)
